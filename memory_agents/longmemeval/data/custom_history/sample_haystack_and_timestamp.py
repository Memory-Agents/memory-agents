...
def generate_random_dates_before(input_date_str, n, days=30):
    ...
    random_dates = [
        one_month_before
        + timedelta(days=random.randint(0, (input_date - one_month_before).days))
        for _ in range(n)
    ]
    # Sort the dates
    random_dates.sort()
...
def generate_random_dates_after(input_date_str, n, days=30):
    ...
    random_dates = [
        input_date
        + timedelta(days=random.randint(1, (one_month_after - input_date).days))
        for _ in range(n)
    ]
    # Sort the dates
    random_dates.sort()
...
def generate_random_dates_in_range(start_date_str, end_date_str, n):
    ...
    # Generate n random dates within the range
    random_dates = [
        start_date + timedelta(days=random.randint(0, delta_days)) for _ in range(n)
    ]
    # Sort the dates
    random_dates.sort()
    return [date.strftime("%Y/%m/%d") for date in random_dates]
...
if len(sys.argv) != 6:
    print(
        "Usage: python sample_haystack_and_timestamp.py task n_questions min_n_haystack_filler max_n_haystack_filler enforce_json_length"
    )
    exit()
task = sys.argv[1]  #'single_hop'
n_questions = int(sys.argv[2])  # 25
min_n_haystack_filler, max_n_haystack_filler = (
    int(sys.argv[3]),
    int(sys.argv[4]),
)  # 10, 10
(
    haystack_source_ratio_user,
    haystack_source_ratio_sharegpt,
    haystack_source_ratio_ultrachat,
) = 0.5, 0.25, 0.25
assert (
    haystack_source_ratio_user
    + haystack_source_ratio_sharegpt
...
suffix = "user{}sharegpt{}ultrachat{}".format(
    haystack_source_ratio_user,
    haystack_source_ratio_sharegpt,
    haystack_source_ratio_ultrachat,
)
if enforce_json_length is not None:
    print(
        "Warning: enforce_json_length is not None, will pack the haystack to {} tokens according to {}".format(
            enforce_json_length, ref_model_name
        )
    )
    out_file = "{}_{}_{}-{}haysess_{}_enflen{}.json".format(
        prefix,
        task,
        min_n_haystack_filler,
        max_n_haystack_filler,
        suffix,
        enforce_json_length,
    )
else:
    out_file = "{}_{}_{}-{}haysess_{}.json".format(
        prefix, task, min_n_haystack_filler, max_n_haystack_filler, suffix
    )
...
qid2attid = {
    x["question_id"]: bgid_to_attribute[x["background_id"]]
    if x["background_id"] in bgid_to_attribute
    else x["background_id"]
...
session_db = json.load(
    open(database_dir + "/6_session_cache/data_6_session_cache.json")
)
sess_idx = {x["session_id"]: x for x in session_db}

haystack_source_sessions_user_raw = [
    x
    for x in json.load(
        open(database_dir + "/6_session_cache/data_6_session_cache.json")
    )
...
haystack_source_sessions_sharegpt = [
    x for x in filler_db if "sharegpt" in x["session_id"]
]
haystack_source_sessions_ultrachat = [
    x for x in filler_db if "ultrachat" in x["session_id"]
]
print(
    "Haystack source: {} user, {} sharegpt, {} ultrachat".format(
        len(haystack_source_sessions_user_all),
        len(haystack_source_sessions_sharegpt),
        len(haystack_source_sessions_ultrachat),
    )
)
...
if task != "assistant_previnfo":
    ...
    for x in question_db:
        ...
        if not [
            y
            for y in x["sessions"]
            if "human_valid_label" in sess_idx[y["session_id"]]
            and sess_idx[y["session_id"]]["human_valid_label"]
        ]:
            continue
        question_candidates.append(x)
else:
    question_candidates = [
        x for x in question_db if x["question_type"] == task and x["sessions"]
    ]
...
for question_entry in tqdm(questions):
    ...
    for x in haystack_source_sessions_user_all:
        if (
            "question_id" not in x
            or x["question_id"] not in qid2attid
            or qid2attid[x["question_id"]] != qid2attid[question_entry["question_id"]]
        ):
            haystack_source_sessions_user_cur_subset.append(x)

    # sample haystack
    cur_n_haystack = random.choice(
        list(range(min_n_haystack_filler, max_n_haystack_filler + 1))
    )
    haystack = []
    ...
    haystack = [
        {"session_id": x["session_id"], "session": x["session"]} for x in haystack
    ]
    ...
    haystack = [
        {
            ...
            "session": [
                {"role": y["role"], "content": y["content"]} for y in x["session"]
            ],
        }
        for x in haystack
    ]
    ...
    if task in ["single_hop", "implicit_preference", "implicit_preference_v2"]:
        ...
        for sess_entry in question_entry["sessions"]:
            if (
                sess_entry["style"] == "neutral"
                and "human_valid_label" in sess_idx[sess_entry["session_id"]]
                and sess_idx[sess_entry["session_id"]]["human_valid_label"]
            ):
        ...
        haystack.append(
            {
                "session_id": "answer_" + answer_session["session_id"] + "_1",
                "session": answer_session["session_1"],
            }
        )
        haystack.append(
            {
                "session_id": "answer_" + answer_session["session_id"] + "_2",
                "session": answer_session["session_2"],
            }
        )
        random.shuffle(haystack)
    ...
    if task in [
        ...
        if "unified_date" in question_entry["question_content"]:
            dates = get_random_same_day_timestamps(
                len(haystack) + 1,
                base_date=question_entry["question_content"]["unified_date"],
            )
        else:
            dates = sorted(
                [get_random_same_day_timestamps(1)[0] for _ in range(len(haystack) + 1)]
            )
        for i in range(len(haystack)):
            haystack[i]["date"] = dates[i]
        question_date = (
            question_entry["question_content"]["question_date"]
        ...
        if "unified_date" in question_entry["question_content"]:
            ...
            unified_base_date = random_date(2023, 2023, 5, 5, 20, 30).strftime(
                "%Y/%m/%d"
            )
        dates = [
            get_random_same_day_timestamps(1, base_date=unified_base_date)[0]
            for _ in range(len(haystack) + 1)
        ]
        for i in range(len(haystack)):
            haystack[i]["date"] = dates[i]
        ...
        for i in range(len(haystack)):
            if "answer_" in haystack[i]["session_id"]:
                haystack[i]["date"] = question_entry["question_content"]["facts"][
                    int(haystack[i]["session_id"][-1]) - 1
                ]["date"]
                # special process
                haystack[i]["date"] = format_date(haystack[i]["date"])
        # hack
        special_session_reordered = sorted(
            [x for x in haystack if "answer_" in x["session_id"]],
            key=lambda x: x["date"],
        )
        ...
        for i in range(len(haystack)):
            ...
            haystack[i]["date"] = get_random_same_day_timestamps(
                1, base_date=haystack[i]["date"]
            )[0]

        question_date = get_random_same_day_timestamps(
            1, base_date=question_entry["question_content"]["question_date"]
        )[0]
        # hack
        ...
        if "temporal_constraint" in question_entry["question_content"]:
            date_old = question_entry["question_content"]["temporal_constraint"][
                "fact_1_date"
            ]
            date_new = question_entry["question_content"]["temporal_constraint"][
                "fact_2_date"
            ]
        else:
        ...
        for hs_entry in haystack:
            if (
                hs_entry["session_id"]
                == "answer_" + answer_session["session_id"] + "_1"
            ):
                left_hit = True
                dates.append(date_old)
            elif (
                hs_entry["session_id"]
                == "answer_" + answer_session["session_id"] + "_2"
            ):
                right_hit = True
    ...
    out_entry = {
        ...
        "answer_session_ids": [
            x["session_id"] for x in haystack if "answer" in x["session_id"]
        ],
    }

    if enforce_json_length:
        while (
            len(tokenizer.encode(json.dumps(out_entry["haystack_sessions"])))
            > enforce_json_length
        ):
            # randomly drop one non-answer session
            filler_indices = [
                i
                for i, s in enumerate(out_entry["haystack_session_ids"])
                if "answer_" not in s
            ]
            remove_idx = random.choice(filler_indices)
            out_entry["haystack_dates"] = (
                out_entry["haystack_dates"][:remove_idx]
                + out_entry["haystack_dates"][remove_idx + 1 :]
            )
            out_entry["haystack_session_ids"] = (
                out_entry["haystack_session_ids"][:remove_idx]
                + out_entry["haystack_session_ids"][remove_idx + 1 :]
            )
            out_entry["haystack_sessions"] = (
                out_entry["haystack_sessions"][:remove_idx]
                + out_entry["haystack_sessions"][remove_idx + 1 :]
            )

    out_token_counts.append(
        len(tokenizer.encode(json.dumps(out_entry["haystack_sessions"])))
    )
...
print(
    "Max token count {}; Min token count {}; Mean token count {}".format(
        max(out_token_counts),
        min(out_token_counts),
        round(sum(out_token_counts) / len(out_token_counts), 2),
    )
)

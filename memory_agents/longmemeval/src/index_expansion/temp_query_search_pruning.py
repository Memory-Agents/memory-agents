        ...
        messages += [
            {
                ...
                "content": user_prompt.format(
                    example_input["date"], example_input["question"]
                ),
            },
...
if __name__ == "__main__":
    ...
    id2timestamp = {
        x["question_id"]: x["timestamped_facts"] for x in in_data_with_timestamp
    }

    out_file = in_retrieval_file + ".timefiltered{}".format(
        model if model == "gpt-4o" else "llama"
    )
    in_retrieval_data = [
        json.loads(line) for line in open(in_retrieval_file).readlines()
    ]
    ...
    for entry in tqdm(in_retrieval_data):
        ...
        event_date_to_sid = {}
        for sid, decomp_events in zip(
            entry["haystack_session_ids"], id2timestamp[entry["question_id"]]
        ):
        ...
        correct_docs = list(
            set(
                [
                    x["corpus_id"]
                    for x in ranked_items_original
                    if "answer" in x["corpus_id"]
                ]
            )
        )
    ...
    print(
        {round(np.mean([x[k] for x in metrics_before]), 4) for k in metrics_before[0]}
    )
    print("Metrics after:")

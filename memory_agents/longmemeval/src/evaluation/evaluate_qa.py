...
if __name__ == '__main__':
    ...
    # 결과 요약 생성
    overall_acc = round(np.mean([1 if x['autoeval_label']['label'] else 0 for x in logs]).item(), 4)
    summary_lines = []
    summary_lines.append(f'Accuracy: {overall_acc}')
    for k, v in qtype2acc.items():
        line = '\t{}: {} ({})'.format(k, round(np.mean(v), 4), len(v))
    ...
    # 파일로 저장
    hyp_basename = os.path.basename(hyp_file)
    summary_file = os.path.join('../../', f'{hyp_basename}.Results_Summary.txt')

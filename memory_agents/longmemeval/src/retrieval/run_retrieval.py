...
def parse_args():
    ...
    parser.add_argument(
        "--granularity", type=str, required=True, choices=["session", "turn"]
    )
...
def batch_get_retrieved_context_and_eval(
    entry_list, args, index_expansion_result_cache=None
):
...
def main(args):
    ...
    out_file = (
        ...
        + "/"
        + outfile_prefix
        + "_retrievallog_{}_{}".format(args.granularity, args.retriever)
    )
    ...
    if "bm25" in args.retriever:
        num_processes = 10
    print(
        "Setting num processes = {} with retriever {}".format(
            num_processes, args.retriever
        )
    )
            ...
            try:
                ...
                for eval_entry in results:
                    ...
                    if not any(
                        ...
                        continue
                    results_list.append(
                        eval_entry["retrieval_results"]["metrics"][t][k]
                    )
                ...
                continue
    print(
        "Ignored {} instances due to abstention: {}".format(
            len(ignored_qs_abstention), ignored_qs_abstention
        )
    )
    print(
        "Additionally ignored {} instances due to no target turns from the user side: {}".format(

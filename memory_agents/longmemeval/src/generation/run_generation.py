...
def parse_args():
    ...
    parser.add_argument(
        "--history_format", type=str, required=True, choices=["json", "nl"]
    )
    parser.add_argument(
        "--useronly", type=str, required=True, choices=["true", "false"]
    )
    parser.add_argument("--cot", type=str, required=True, choices=["true", "false"])
    parser.add_argument(
        "--con", type=str, required=False, choices=["true", "false"], default="false"
    )
    ...
    parser.add_argument(
        ...
        default="none",
    )  # merge key expansion into value
...
def prepare_prompt(
    ...
    if retriever_type == "no-retrieval":
        ...
        if (
            merge_key_expansion_into_value is None
            or merge_key_expansion_into_value == "none"
        ):
            if cot:
    ...
    if retriever_type == "orig-session":  # no retrieval, session
        for session_date, session_entry in zip(
            entry["haystack_dates"], entry["haystack_sessions"]
        ):
            if useronly:
    ...
    # get retrieved chunks
        ...
        for ret_result_entry in entry["retrieval_results"]["ranked_items"]:
            converted_corpus_id = "_".join(
                ret_result_entry["corpus_id"]
                .replace("noans_", "answer_")
                .split("_")[:-1]
            )
            converted_turn_id = (
                int(
                ...
                - 1
            )  # we had offset one during retrieval
            # automatically expand turn into round
            try:
                cur_round_data = [
                    corpusid2entry[converted_corpus_id][converted_turn_id]
                ]
            ...
            if (
                ...
                retrieved_chunks.append(
                    (corpusid2date[converted_corpus_id], cur_round_data)
                )
            elif merge_key_expansion_into_value == "replace":
                retrieved_chunks.append(
                    (
                        corpusid2date[converted_corpus_id],
                        corpusid2retvalue[ret_result_entry["corpus_id"]],
                    )
                )
            elif merge_key_expansion_into_value == "merge":
                retrieved_chunks.append(
            ...
            if (
                ...
                if useronly:
                    retrieved_chunks.append(
                        (
                            corpusid2date[
                                ret_result_entry["corpus_id"].replace(
                                    "noans_", "answer_"
                                )
                            ],
                            [
                                x
                                for x in corpusid2entry[
                                    ret_result_entry["corpus_id"].replace(
                                        "noans_", "answer_"
                                    )
                                ]
                ...
                retrieved_chunks.append(
                    (
                        corpusid2date[
                            ret_result_entry["corpus_id"].replace("noans_", "answer_")
                        ],
                        corpusid2retvalue[ret_result_entry["corpus_id"]],
                    )
                )
    ...
    if con:
        ...
        for i, cur_item in enumerate(retrieved_chunks):
            ...
            if merge_key_expansion_into_value == "merge":
                retrieved_chunks_with_notes.append(
                    (chunk_date, chunk_expansion_entry, chunk_entry_con)
                )
            else:
    ...
    for i, cur_item in enumerate(retrieved_chunks):
        ...
        if history_format == "json":
            if merge_key_expansion_into_value == "merge":
                sess_string = "\n" + json.dumps(
                    {
                        "session_summary_facts": chunk_expansion_entry,
                        "original_session": chunk_entry,
                    }
                )
            else:
                sess_string = "\n" + json.dumps(chunk_entry)
            ...
            if type(chunk_entry) == list:
                ...
                sess_string += "{}: {}".format(
                    chunk_entry["role"], chunk_entry["content"].strip()
                )
        else:
            raise NotImplementedError
        ...
        if retriever_type in ["orig-session", "flat-session", "oracle-session"]:
            ...
            )  # we include both sides right now
        elif retriever_type == "no-retrieval":
            history_string += ""
        else:
            raise NotImplementedError
    ...
    if retriever_type == "no-retrieval":
        ...
        if tokenizer_backend == "openai":
            ...
            if len(tokens) > max_retrieval_length:
                print(
                    "Truncating from {} to {}".format(
                        len(tokens), max_retrieval_length
                    ),
                    flush=True,
                )
            ...
            encoded_input = tokenizer(
                history_string,
                max_length=max_retrieval_length,
                truncation=False,
                return_tensors="pt",
            )
            if len(encoded_input["input_ids"][0]) > max_retrieval_length:
                print(
                    "Truncating from {} to {}".format(
                        len(encoded_input["input_ids"][0]), max_retrieval_length
                    )
                )
                encoded_input = tokenizer(
                    history_string,
                ...
                history_string = tokenizer.decode(
                    encoded_input["input_ids"][0], skip_special_tokens=True
                )
        else:
            raise NotImplementedError
        prompt = answer_prompt_template.format(
            history_string, question_date_string, question_string
        )
...
def main(args):
    ...
    if (
        args.merge_key_expansion_into_value is not None
        and args.merge_key_expansion_into_value != "none"
    ):
        out_file = (
            args.out_dir
            + "/"
            + in_file_tmp
            + "_testlog_top{}context_{}format_useronly{}_factexpansion{}_{}".format(
                args.topk_context,
                args.history_format,
                args.useronly,
                args.merge_key_expansion_into_value,
        ...
        out_file = (
            ...
            + "/"
            + in_file_tmp
            + "_testlog_top{}context_{}format_useronly{}_{}".format(
                args.topk_context,
                args.history_format,
                args.useronly,
                datetime.now().strftime("%Y%m%d-%H%M"),
    ...
    for entry in tqdm(in_data):
        ...
        try:
            ...
            print(
                json.dumps({"question_id": entry["question_id"], "hypothesis": answer}),
                file=out_f,
                flush=True,
            )
